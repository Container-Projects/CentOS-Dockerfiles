...
function _M.filter()
    ...
    local base_uri = rangerConfig.configs['base_uri']
    local dashboard_host = rangerConfig.configs['dashboard_host']
    ...
    if string.find( uri, base_uri ) == 1 then
        ...
        ngx.var.vn_static_root = rangerConfig.home_path() .."/dashboard"
...
function _M.check_session()
    ...
    local cookie_prefix = rangerConfig.configs['cookie_prefix']
    ...
    for i,v in ipairs( rangerConfig.configs['admin'] ) do
...
_M.route_table = {
    ...
    { ['method'] = "GET",  ['auth']= true,  ["path"] = "/config", ['handle'] = rangerConfig.report },
    { ['method'] = "POST", ['auth']= true,  ["path"] = "/config", ['handle'] = rangerConfig.set },
    { ['method'] = "GET",  ['auth']= true,  ["path"] = "/loadconfig", ['handle'] = rangerConfig.load_from_file },
}
