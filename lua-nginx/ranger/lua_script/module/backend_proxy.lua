...
function _M.filter()
    ...
    if rangerConfig.configs["proxy_pass_enable"] ~= true then
        return
    end

    local matcher_list = rangerConfig.configs['matcher']
    local upstream_list = rangerConfig.configs['backend_upstream']

    for i, rule in ipairs( rangerConfig.configs["proxy_pass_rule"] ) do
        ...
        if enable == true and request_tester.test( matcher ) == true then
            ...
            return ngx.exec('@backend_proxy')  --will jump out at the exec, so the return not run in fact
        end
    end
end
