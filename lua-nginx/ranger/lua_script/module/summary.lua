...
function _M.refresh()
    local period = tonumber( rangerConfig.configs["summary_request_enable"] )
...
function _M.pre_run_matcher()
    if rangerConfig.configs["summary_request_enable"] ~= true or rangerConfig.configs["summary_collect_enable"] ~= true then
        return
    end

    local matcher_list = rangerConfig.configs['matcher']
    for i,rule in ipairs( rangerConfig.configs["summary_collect_rule"] ) do
...
function _M.log()
    ...
    if rangerConfig.configs["summary_request_enable"] ~= true then
        return
    end

    local with_host_info = rangerConfig.configs["summary_with_host"]
    ...
    if rangerConfig.configs["summary_group_persistent_enable"] == true then
    ...
    if rangerConfig.configs["summary_group_temporary_enable"] == true then
